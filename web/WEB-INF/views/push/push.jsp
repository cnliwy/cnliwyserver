...
<div class="container" style="margin-top: 80px;">
    <div class="row clearfix">
        <div class="col-md-12 column">
            <form class="navbar-form navbar-left" id="form-add" role="search" action="${basePath}/push/add" method="post">
                <div class="form-group">
                    <input type="text" name="notifyTitle" id="notifyTitle" class="form-control" placeholder="通知标题"/>
                    <input type="text" name="notifyContent" id="notifyContent" class="form-control" placeholder="通知内容"/>
                </div> <button type="button" class="btn btn-default" onclick="push()">推送</button>
                ...
                <a href="#div-help" data-toggle="modal">推送使用帮助</a>
            </form>
            <table class="table table-hover">
                ...
                <tbody>
                <c:forEach items="${pushDatas}" var="data" varStatus="status">
                    ...
                    <c:if test="${status.index % 2 > 0}">
                        ...
                        <td  id="${data.id}"> ${data.notifyContent}</td>
                        <td>${data.pushTime}</td>
                        <td>
...
<script>
    ...
    function deletePushData(id) {
        $.ajax({
            type:"post",
            url : "${basePath}/push/delete?id=" + id,
            dataType : "json",
            success : function (data) {
                if(data.state == 0){
                    alert("删除成功")
